...
feat_name=$3
test -z "$feat_name" && feat_name=ComParE2015_Parkinson_mfc18_elasso_f20
...
# train svr using Weka's SMOreg, using FilteredClassifier wrapper to ignore first attribute (instance name)
     ...
     java -Xmx$jvm_mem -classpath $weka_jar weka.classifiers.meta.FilteredClassifier -v -o -no-cv -c last -t "$train_libsvm" -d "$svr_model_name" -F "weka.filters.unsupervised.attribute.Remove -R 1" -W weka.classifiers.functions.SMOreg -- -C $C -N 1 -I "weka.classifiers.functions.supportVector.RegSMOImproved -L $L -W 1 -P 1.0E-12 -T 0.001 -V" -K "weka.classifiers.functions.supportVector.PolyKernel -C 250007 -E 1.0" || exit 1
#fi
