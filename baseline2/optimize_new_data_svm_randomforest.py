...
for group in range(5):
    ...
    TRAIN = ["Experiment_New_Data_"+str(group+1)+"_updrs_"+version, "Experiment_New_Data_"+str(group+1)+"_updrs_"+version+"_elasso"]
    os.system("rm print_new_data_svm_randomforest_"+str(group+1)+"_"+version+".dep")
    ...
    for n in range(len(script_bash)):
        ...
        if os.path.exists(script_bash[n]):
            ...
            for cont in range(len(params)):
                ...
                for i in range(len(params)):
                    parallel += string[i]
                f = open("print_new_data_svm_randomforest_"+str(group+1)+"_"+version+".dep", "a")
                f.write("Training " + parallel + "\n" + "\n")
                f.close()
                os.system(parallel)
                for exp in range(experiments[cont]):
                    params[cont] = values[exp]
                    if os.path.exists('eval/train_devel/'+TRAIN[n]+'.SVR.C'+str(params[0])+'.L'+str(params[1])+'.pred'):
                        file = open('eval/train_devel/'+TRAIN[n]+'.SVR.C'+str(params[0])+'.L'+str(params[1])+'.pred', 'r')
                        data = file.readlines();
                        valor = []
                        pred = []
                        for x in data:
            ...
            file = open('eval/train_devel/'+TRAIN[n]+'.SVR.C0.001.L1.0.pred', 'r')
            data = file.readlines();
            valor = []
            pred = []
            err = []
    ...
    for n in range(len(script_bash)):
        ...
        if os.path.exists(script_bash[n]):
            ...
            for cont in range(len(params)):
                ...
                for exp in range(experiments[cont]):
                    params[cont] = values[exp]
                    if os.path.exists('eval/train_devel/'+TRAIN[n]+'.RandomForest.I'+str(params[0])+'.S'+str(params[1])+'.pred'):
                        file = open('eval/train_devel/'+TRAIN[n]+'.RandomForest.I'+str(params[0])+'.S'+str(params[1])+'.pred', 'r')
                        data = file.readlines();
                        valor = []
                        pred = []
                        for x in data:
                        ...
                        spearman_comparison.append(-1)
                        f = open("print_new_data_svm_randomforest_"+str(group+1)+"_"+version+".dep", "a")
                        f.write("The result file for I="+str(params[0])+" and S="+str(params[1])+" has not been created" + "\n")
                        f.close()
                index = spearman_comparison.index(max(spearman_comparison))
                params[cont] = values[index]
            ...
            file = open('eval/train_devel/'+TRAIN[n]+'.RandomForest.I100.S1.pred', 'r')
            data = file.readlines();
            valor = []
            pred = []
            err = []
