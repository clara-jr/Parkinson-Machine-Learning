...
if os.path.exists('baseline_train_devel_new_data.sh'):
    ...
    os.system("rm print_new_data.dep")

    for cont in range(len(params)):
        values = []
        spearman = []
        string = []
        parallel = "parallel -j " + str(experiments[cont]) + " ./baseline_train_devel_new_data.sh {1} {2}"
        ...
        for exp in range(experiments[cont]):
            ...
            if os.path.exists('eval/train_devel/New_Data.SVR.C'+str(params[0])+'.L'+str(params[1])+'.result'):
                file = open('eval/train_devel/New_Data.SVR.C'+str(params[0])+'.L'+str(params[1])+'.result', 'r')
                data = file.readlines();
                for x in data:
                    line = x.split(" ");
                s = line[len(line)-1]
    ...
    f.close()
    os.system('./baseline_train_devel_new_data.sh '+str(params[0])+' '+str(params[1]))
    file = open('eval/train_devel/New_Data.SVR.C'+str(params[0])+'.L'+str(params[1])+'.result', 'r')
    data = file.readlines();
    for x in data:
        line = x.split(" ");
    ...
    f.close()
    os.system('./baseline_train_devel_new_data.sh 0.001 1.0')
    file = open('eval/train_devel/New_Data.SVR.C0.001.L1.0.result', 'r')
    data = file.readlines();
    for x in data:
        line = x.split(" ");
    s = line[len(line)-1]
