...
# model file name
svr_model_name=$model_dir/$feat_name.train.SimpleLinearRegression.model
...
# train svr using Weka's SMOreg, using FilteredClassifier wrapper to ignore first attribute (instance name)
     ...
     java -Xmx$jvm_mem -classpath $weka_jar weka.classifiers.meta.FilteredClassifier -v -o -no-cv -c last -t "$train_arff" -d "$svr_model_name" -F "weka.filters.unsupervised.attribute.Remove -R 1" -W  weka.classifiers.functions.SimpleLinearRegression || exit 1
#fi

echo "finished train model" >> pruebas.dep

# evaluate svr and write predictions
pred_file=$eval_dir/$feat_name.SimpleLinearRegression.pred
#if [ ! -s "$pred_file" ]; then
...
# test predictions
pred_test_file=$eval_dir/$feat_name.SimpleLinearRegression.test.pred
