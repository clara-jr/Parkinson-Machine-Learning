    ...
    if os.path.exists("baseline_svm_arff.sh"):
        ...
        for cont in range(len(params)):
            ...
            for exp in range(experiments[cont]):
                ...
                if os.path.exists('eval/train_devel/'+TRAIN+'.SVR.C'+str(params[0])+'.L'+str(params[1])+'.pred'):
                    file = open('eval/train_devel/'+TRAIN+'.SVR.C'+str(params[0])+'.L'+str(params[1])+'.pred', 'r')
                    data = file.readlines();
                    err = []
                    valor = []
                    for x in data:
        ...
        file = open('eval/train_devel/'+TRAIN+'.SVR.C0.001.L1.0.pred', 'r')
        data = file.readlines();
        err = []
        valor = []
        for x in data:
