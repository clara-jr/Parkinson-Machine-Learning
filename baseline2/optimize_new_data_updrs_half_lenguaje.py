...
if os.path.exists('baseline_svm_arff.sh'):
    ...
    for park_class in range(len(parkinson_class)):
        ...
        for n in range(len(parkinson_experiments)):
            ...
            os.system("rm print_new_data_"+parkinson_experiments[n]+"_"+parkinson_class[park_class]+"_"+version+"_categorise.dep")
            ...
            for cont in range(len(params)):
                ...
                for i in range(len(params)):
                    parallel += string[i]
                f = open("print_new_data_"+parkinson_experiments[n]+"_"+parkinson_class[park_class]+"_"+version+"_categorise.dep", "a")
                ...
                for exp in range(experiments[cont]):
                    ...
                    if os.path.exists('eval/train_devel/'+FEATURE+'.SVR.C'+str(params[0])+'.L'+str(params[1])+'.pred'):
                        ...
                        for locutor in range(n_locutores/n_grupos[n]):
                            ...
                            if round(pred_locutor[locutor]) > 4.0:
                                pred_locutor[locutor] = 4.0
                            elif round(pred_locutor[locutor]) < 0.0:
                                pred_locutor[locutor] = 0.0
                            else:
