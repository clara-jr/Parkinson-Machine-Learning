...
if os.path.exists('extract.sh'):
    ...
    train = open("features/New_Data_3.train.arff", "a")
    devel = open("features/New_Data_3.devel.arff", "a")
    for group in range(5):
        for pacient in range(5):
            for audio in range(48):
                if group != 2:
                    if group == 0 and pacient==0 and audio==0:
                        os.system("cp features/palabras_1_" + str(index_sort[pacient+group*5]) + ".arff features/New_Data_3.train.arff")
                    else:
                    ...
                    if pacient==0 and audio==0:
                        os.system("cp features/palabras_1_" + str(index_sort[pacient+group*5]) + ".arff features/New_Data_3.devel.arff")
                    else:
                        file = open("features/palabras_" + str(audio+1) + "_" + str(index_sort[pacient+group*5]) + ".arff", 'r')
