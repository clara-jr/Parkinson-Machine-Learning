...
hoenhyahr = [25, 69, 0, 49, 66, 34, 27, 18, 38, 52, 56, 57, 74, 71, 50, 55, 87, 24, 23, 54, 59, 21, 22, 19, 6, 48]
...
if os.path.exists('extract.sh'):
    ...
    for group in range(5):
        data = open("features/New_Data_" + str(group) + ".arff", "a")
        for pacient in range(5):
            print str(index_sort[pacient+group*5])
            for audio in range(48):
                if pacient==0 and audio==0:
                    os.system("cp features/palabras_1_" + str(index_sort[pacient+group*5]) + ".arff features/New_Data_" + str(group) + ".arff")
                else:
