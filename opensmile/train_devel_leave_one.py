...
if os.path.exists('extract.sh'):
    ...
    for pacient_devel in range(len(index_sort)):
        ...
        os.system("cp features/palabras_1_" + str(index_sort[pacient_devel]) + ".arff features/Experiment_New_Data_"+str(index_sort[pacient_devel])+"_leave_one.devel.arff")
        for audio in range(48):
            if audio != 0:
                file_devel = open("features/palabras_" + str(audio+1) + "_" + str(index_sort[pacient_devel]) + ".arff", 'r')
                devel.write(file_devel.readlines()[-1])
                file_devel.close()
        devel.close()
        if pacient_devel != 0:
            os.system("cp features/palabras_1_0.arff features/Experiment_New_Data_"+str(index_sort[pacient_devel])+"_leave_one.train.arff")
        else:
            pacient_train_inic = 1
            os.system("cp features/palabras_1_1.arff features/Experiment_New_Data_"+str(index_sort[pacient_devel])+"_leave_one.train.arff")
        for pacient_train in range(len(index_sort)):
            if pacient_train != pacient_devel:
                if pacient_train != pacient_train_inic:
                    for audio in range(48):
                        file_train = open("features/palabras_" + str(audio+1) + "_" + str(index_sort[pacient_train]) + ".arff", 'r')
