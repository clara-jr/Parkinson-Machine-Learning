...
audio_train = [1, 2, 5, 6, 9, 10, 13, 14, 17, 18, 21, 22, 25, 26, 29, 30, 33, 34, 37, 38, 41, 42, 45, 46]
audio_devel = [3, 7, 11, 15, 19, 23, 27, 31, 35, 39, 43, 47]
audio_test = [4, 8, 12, 16, 20, 24, 28, 32, 36, 40, 44, 48]
n_audios = 48
...
if os.path.exists('extract.sh'):
    ...
    for pacient in range(len(index_sort)):
        for audio_train in range(len(audio_train)):
            if pacient == 0:
                if audio != 0:
                    file_train = open("features/palabras_" + str(audio+1) + "_" + str(index_sort[pacient]) + "_"+parkinson_class+".arff", 'r')
                ...
                train.write(file_train.readlines()[-1])
                file_train.close()
        for audio_devel in range(len(audio_devel)):
            if pacient == 0:
                if audio != 2:
                    file_devel = open("features/palabras_" + str(audio+1) + "_" + str(index_sort[pacient]) + "_"+parkinson_class+".arff", 'r')
                ...
                devel.write(file_devel.readlines()[-1])
                file_devel.close()
        for audio_test in range(len(audio_test)):
            if pacient == 0:
                if audio != 3:
                    file_test = open("features/palabras_" + str(audio+1) + "_" + str(index_sort[pacient]) + "_"+parkinson_class+".arff", 'r')
